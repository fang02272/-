...
MANUAL_DEFINITIONS = {
    ...
    # === v2.6 新增概念定义 ===
    "碳当量": {
        "definition": "碳当量（CE/Ceq）：把钢中合金元素对淬硬/冷裂倾向的影响折算成碳的等效含量。常用公式 CE=C+Mn/6+(Cr+Mo+V)/5+(Ni+Cu)/15。CE 越高，冷裂倾向越大，需预热程度越高。",
        "application": "应用要点：CE<0.4 易焊；0.4-0.6 需预热；>0.6 严格控氢+后热。用于高强钢焊接工艺评定。",
        "process_types": [],
    },
    "冷裂纹敏感性指数": {
        "definition": "冷裂纹敏感性指数（Pcm）：针对含微量合金的高强钢的冷裂敏感评估公式 Pcm=C+Si/30+(Mn+Cu+Cr)/20+Ni/60+Mo/15+V/10+5B。比碳当量更精确，用于现代高强钢。",
        "application": "应用要点：Pcm 值越高冷裂倾向越大；用于确定预热温度与低氢焊材要求。",
        "process_types": [],
    },
    "射线探伤": {
        "definition": "射线探伤（RT）：用 X 射线/γ 射线穿透工件，胶片/探测器记录缺陷影像，检测焊缝内部缺陷（气孔/夹渣/未熔合/裂纹）的方法。直观、可留档，适合厚板/承压件。",
        "application": "应用要点：压力容器/管道按标准 RT 抽检；射线方向垂直缺陷面；铅防护安全。",
        "process_types": [],
    },
    "磁粉探伤": {
        "definition": "磁粉探伤（MT）：对铁磁性工件磁化，缺陷处漏磁吸附磁粉形成磁痕，检测表面/近表面缺陷（裂纹/发纹）。仅适用铁磁性材料，灵敏度高。",
        "application": "应用要点：焊缝表面裂纹检测常用；工件需消磁；表面打磨光亮。",
        "process_types": [],
    },
    "渗透探伤": {
        "definition": "渗透探伤（PT）：将渗透液涂敷表面渗入开口缺陷，清除后显像剂吸出，检测表面开口缺陷。适用所有材料，不受磁性限制。",
        "application": "应用要点：不锈钢/铝/陶瓷表面裂纹检测；表面需清洁干燥；着色法/荧光法。",
        "process_types": [],
    },
    "涡流检测": {
        "definition": "涡流检测（ET）：线圈交变磁场在导电工件中感应涡流，缺陷改变涡流分布被检测。适合管材/线材/薄板的表面与近表面缺陷快速检测。",
        "application": "应用要点：非接触高速检测；适用导电材料；对表面粗糙敏感。",
        "process_types": [],
    },
    "目视检测": {
        "definition": "目视检测（VT）：直接用肉眼/放大镜/内窥镜观察焊缝表面缺陷（咬边/裂纹/焊瘤/成形不良）的检测方法。是最基本、最先行的检测手段。",
        "application": "应用要点：焊前坡口检查/焊后外观检验；配合照明与测量工具。",
        "process_types": [],
    },
    "焊接变形": {
        "definition": "焊接变形：焊接不均匀加热冷却产生的残余应力导致工件尺寸/形状变化（收缩/角变形/弯曲/扭曲/波浪变形）。由热输入、拘束、焊接顺序决定。",
        "application": "应用要点：对称焊/分段退焊/反变形法/刚性固定；机器人焊对称排道控线能量；焊后校形。",
        "process_types": [],
    },
    "药芯焊丝焊": {
        "definition": "药芯焊丝焊（FCAW）：使用管状药芯焊丝，药芯造渣/造气保护熔池，兼有焊条药皮保护和焊丝连续送丝高效率。适合户外/大电流/全位置。",
        "application": "应用要点：机器人 FCAW 用于中厚板/结构件；注意清渣与烟尘；选低飞溅药芯焊丝。",
        "process_types": ["FCAW (药芯焊丝CO₂焊)"],
    },
    "船型焊": {
        "definition": "船型焊（PA 位置/船形焊）：将工件旋转至坡口朝上（约45°），使焊缝处于水平位置焊接的方式。熔池水平、重力不影响流动，成形最稳，是机器人角焊缝最理想位置。",
        "application": "应用要点：机器人焊接优先船型焊位（坡口转45°朝上）；焊枪近似垂直（工作角90°）；角焊缝/对接缝均适用。",
        "process_types": [],
    },
    "焊枪姿态": {
        "definition": "焊枪姿态：焊枪相对工件的位置与角度，分解为工作角（绕焊接方向）、行走角（沿焊接方向推/拉枪）、绕枪轴旋转。姿态决定熔深/成形/飞溅。",
        "application": "应用要点：机器人焊接按工艺定姿态：平焊 10-15°推枪、工作角 75-85°；管道/圆弧处随轨迹调整。",
        "process_types": [],
    },
    "干伸长": {
        "definition": "干伸长（stick out/导电嘴距离）：焊丝从导电嘴伸出到工件的长度，影响电流密度与熔深。过长电弧不稳飞溅大，过短易回烧导电嘴。",
        "application": "应用要点：MIG 约 12-15mm、FCAW 15-25mm；机器人焊接保持恒定干伸长保证一致成形。",
        "process_types": [],
    },
    "送丝速度": {
        "definition": "送丝速度：熔化极焊中焊丝连续送进的速度，与电流成正比（等速送丝系统）。决定熔敷率与熔深，是机器人焊接关键参数。",
        "application": "应用要点：送丝速度与电流电压匹配；薄板低送丝、厚板高送丝；机器人精确控送丝保证熔敷稳定。",
        "process_types": [],
    },
    "焊缝成形": {
        "definition": "焊缝成形：焊缝截面形状与表面质量（余高/熔宽/熔深/波纹），受电流/电压/焊速/姿态影响。良好成形=均匀余高、无咬边气孔、波纹细密。",
        "application": "应用要点：机器人焊通过参数匹配+姿态稳定控成形；摆动/脉冲改善成形。",
        "process_types": [],
    },
    "熔深": {
        "definition": "熔深：焊缝熔化深度（母材熔入深度），决定接头强度与焊透程度。由电流/焊速/坡口决定，过浅未焊透、过深烧穿。",
        "application": "应用要点：打底焊需保证熔透；机器人焊控电流与焊速稳定熔深。",
        "process_types": [],
    },
    "背面成形": {
        "definition": "背面成形：焊缝背面（根部）的焊透与成形状态，全熔透焊背面应有均匀焊瘤/焊透。背面成形差导致未焊透/内凹。",
        "application": "应用要点：打底焊/单面焊双面成形技术；气体保护背面（管道用背面氩气）；机器人焊控根部熔透。",
        "process_types": [],
    },
    "坡口角度": {
        "definition": "坡口角度：坡口两侧面夹角（V 形约 60°、X 形每侧 30°），决定填充量与熔透条件。角度过小难熔透、过大填充多变形大。",
        "application": "应用要点：按板厚选坡口角；机器人焊缝跟踪检测坡口角规划填充层道。",
        "process_types": [],
    },
    "飞溅": {
        "definition": "飞溅：焊接过程中熔滴/熔池喷溅出的金属颗粒，粘附工件表面，影响成形与外观。由电流电压不匹配、短路过渡不当、保护气不足引起。",
        "application": "应用要点：MIG 脉冲/优化参数减少飞溅；机器人焊控电压电流匹配；飞溅清除增加工时。",
        "process_types": [],
    },
    "引弧": {
        "definition": "引弧：建立焊接电弧的过程（接触引弧/非接触引弧）。引弧不稳影响起焊质量（未熔合/弧坑）。",
        "application": "应用要点：机器人焊接精确引弧（定点引弧+预热）；GTAW 高频引弧；引弧处补焊。",
        "process_types": [],
    },
    "收弧": {
        "definition": "收弧：焊接结束熄弧的过程，处理不当会产生弧坑裂纹/缩孔。需填满弧坑后熄弧。",
        "application": "应用要点：机器人焊自动收弧（回填弧坑+缓降电流）；收弧处打磨补焊。",
        "process_types": [],
    },
    "弧坑": {
        "definition": "弧坑：焊缝收尾处因熄弧形成的凹陷，易产生弧坑裂纹/缩孔，削弱接头。",
        "application": "应用要点：收弧回填/延送丝填满弧坑；机器人焊自动收弧程序。",
        "process_types": [],
    },
    "焊剂": {
        "definition": "焊剂（flux）：埋弧焊/电渣焊中覆盖电弧的颗粒状材料，熔化造渣/造气保护熔池，并参与冶金反应（合金化/脱氧）。",
        "application": "应用要点：按焊丝匹配焊剂（SJ101/HJ431）；焊剂需烘干防气孔；回收筛分复用。",
        "process_types": [],
    },
    "焊接工艺评定": {
        "definition": "焊接工艺评定：按标准（GB/T 50661、ISO 15614、AWS D1.1）对拟定的焊接工艺（WPS）进行试焊与检验，验证其能获得合格接头的过程，形成 PQR 记录。是压力容器/钢结构焊接前必须完成的程序。",
        "application": "应用要点：评定合格后编写 WPS 指导生产；机器人焊接工艺变更（材料/板厚/工艺）需重新评定。",
        "process_types": [],
    },
    "焊后消氢": {
        "definition": "焊后消氢（消氢处理）：焊后立即对焊缝加热保温（如 250°C×2h），促使扩散氢逸出，防止氢致冷裂纹。是厚板高强钢焊接防裂的重要措施。",
        "application": "应用要点：高强钢/厚板焊后立即消氢；与预热/层间温度配合；消氢后需后热处理时再升温。",
        "process_types": [],
    },
    "焊缝金属": {
        "definition": "焊缝金属（weld metal）：由填充金属（焊丝/焊条）与母材熔化混合后凝固形成的焊缝部分。其成分/组织决定焊缝强度/韧性/耐蚀性，受母材稀释与冶金反应影响。",
        "application": "应用要点：按母材+性能要求选填充金属；控稀释率（多层多道/坡口设计）；焊缝金属组织（针状铁素体）影响韧性。",
        "process_types": [],
    },
    "熔滴": {
        "definition": "熔滴：焊条/焊丝端部熔化的金属液滴，以不同过渡形式（短路/颗粒/射流/射滴）进入熔池。熔滴大小/频率影响电弧稳定、飞溅与成形。",
        "application": "应用要点：脉冲 MIG 控熔滴过渡减少飞溅；射流过渡熔滴细小高速适合厚板；短路过渡适合薄板全位置。",
        "process_types": ["GMAW/MIG (熔化极氩弧焊)"],
    },
    "焊丝": {
        "definition": "焊丝：连续送进的填充金属丝，用于熔化极气体保护焊（GMAW/FCAW）等。材质与母材匹配（ER50-6 低碳钢、ER308L 不锈钢、ER5356 铝），直径 0.8-1.6mm。",
        "application": "应用要点：焊丝直径/送丝速度决定电流；机器人焊用盘装焊丝连续送丝；按母材选焊丝合金体系。",
        "process_types": ["GMAW/MIG (熔化极氩弧焊)"],
    },
    "焊接位置": {
        "definition": "焊接位置：焊缝在空间的位置，用代号表示：平焊（PA/1G）、横焊（PB/2G）、立焊（PF/3G）、仰焊（PE/4G）、全位置（管道 5G/6G）。不同位置需调整参数与操作。",
        "application": "应用要点：平焊最易、机器人优先；立焊/仰焊调整电流与摆动；管道固定焊（6G）分半圈分段。",
        "process_types": [],
    },
    "焊道": {
        "definition": "焊道：一次焊接形成的焊缝单元（焊条/焊丝一次连续熔敷）。多层多道焊由多个焊道组成，焊道排布影响熔透/成形/残余应力。",
        "application": "应用要点：打底/填充/盖面焊道规划；焊道宽度≤3倍焊丝直径；机器人焊按坡口排道。",
        "process_types": [],
    },
}
